...
public class Parser {
     ...
     * factor is higher precedence than term.
     * When we hit a term (a + b) we evaluate both `a` and `b` as factors.
     * Meaning if b = c * d then our ast would be
     *          [+]
     *         /   \
     *        a    [*]
     *             / \
    ...
    /** This while loop allows for multiple function calls: getAdder()(2, 4) */
    private Expr call() {
        Expr expr = primary();
    ...
    private Expr primary() {
        ...
        if (match(READ)) return new Expr.Read(previous());
